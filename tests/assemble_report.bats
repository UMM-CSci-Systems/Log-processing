...
setup() {
  ...
  cp tests/username_dist.html "$BATS_TMPDIR"
  cp tests/hours_dist.html "$BATS_TMPDIR"
  cp tests/country_dist.html "$BATS_TMPDIR"
}
...
@test "bin/assemble_report.sh generates correct simple output" {
  run bin/assemble_report.sh "$BATS_TMPDIR"
  run diff -wbB tests/failed_login_summary.html "$BATS_TMPDIR"/failed_login_summary.html
  [ "$status" -eq 0 ]
}
